...
def enablePrivilege(priv, handle):
    ...
    # Setup a PRIVILEGE_SET for the PrivilegeCheck Call to be used later
    requiredPrivileges = PRIVILEGE_SET()
    requiredPrivileges.PrivilegeCount = 1  # Only looking at 1 Privilege at a time here
    ...
    # Check LUID is setup and pointing to the correct Privilege check to see if its enabled
    ...
    # Check pfResult to see if our Privilege is enabled or not
    if pfResult:
    ...
    # Don't attempt to modify the selected Privilege in the Token
...
def openProcessByPID(pid):
    ...
    # Check to see if there's a valid Handle to the process
    if hProcess <= 0:
...
# Spawn A Process As that Current User!
# Use the Win API Call CreateProcessWithTokenW
...
lpStartupInfo.wShowWindow = 0x1  # Show a window (Don't have to)
...
print("[INFO] Created Impersonated Process xD")
